						...
						const loop = async () => {
							...
							if (target && !processing) {
								...
								if (changed) {
									processing = true;
									try {
										await commands.updateCameraOverlayBounds(target.x, target.y, target.width, target.height);
										lastApplied = target;
									} catch (e) {
					...
					createEffect(async () => {
						...
						if (was && !interacting) {
							if (options.mode === "screenshot" && isValid()) {
								...
								console.log("[Screenshot Debug] window.innerWidth/Height:", window.innerWidth, window.innerHeight);
								...
								console.log("[Screenshot Debug] target being sent:", JSON.stringify(target, null, 2));
